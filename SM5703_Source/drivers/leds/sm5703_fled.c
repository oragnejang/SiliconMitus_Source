                         ...
                         const char *buf, size_t count)
{
	...
	if(*buf == '0') {
        ...
        sm5703_assign_bits(client, SM5703_FLEDCNTL1,SM5703_FLEDEN_MASK,SM5703_FLEDEN_DISABLE);//disable flash/movie
	} else if(*buf == '1'){
...
#endif
        sm5703_assign_bits(client, SM5703_FLEDCNTL1,SM5703_FLEDEN_MASK,SM5703_FLEDEN_EXTERNAL);//disable movie
		gpio_request(led_irq_gpio1, NULL);
		...
		pr_err("Movie Factory : led_irq_gpio1(%d)_irq_gpio2(%d)\n", led_irq_gpio1, led_irq_gpio2);
        sm5703_assign_bits(client, SM5703_FLEDCNTL1,SM5703_FLEDEN_MASK,SM5703_FLEDEN_EXTERNAL);//disable movie        
		gpio_request(led_irq_gpio1, NULL);
		gpio_direction_output(led_irq_gpio1, 1);
		gpio_free(led_irq_gpio1);
...
static int sm5703_fled_init(struct sm_fled_info *fled_info)
{
	...
	info->base.flashlight_dev->props.flash_brightness =
	...
	info->base.flashlight_dev->props.movie_brightness =
...
static struct flashlight_properties sm5703_fled_props = {
	...
	.movie_brightness = 0,
	.movie_max_brightness = 0x1f,
	.flash_brightness = 0,
	.flash_max_brightness = 0x1f,
	.flash_delay = 2,
	//.flash_timeout = 64,
		...
		int selector)
{
	...
	if (selector < 0 || selector >  info->
	    base.flashlight_dev->props.movie_max_brightness)
		return -EINVAL;
	rc = sm5703_assign_bits(info->i2c_client, SM5703_FLEDCNTL4,
				SM5703_IMLED_MASK, selector);
	if (rc == 0)
		info->base.flashlight_dev->props.movie_brightness = selector;
	return rc;
}
		...
		int selector)
{
	...
	if (selector < 0 || selector >  info->
	    base.flashlight_dev->props.flash_max_brightness)
		return -EINVAL;
	rc = sm5703_assign_bits(info->i2c_client, SM5703_FLEDCNTL3,
				SM5703_IFLED_MASK, selector);
	if (rc == 0)
		info->base.flashlight_dev->props.flash_brightness = selector;
	return 0;
}
