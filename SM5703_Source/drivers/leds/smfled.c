...
{
	sm_fled_info_t *info = flashlight_get_data(flashlight_dev);
	return info->hal->fled_flash(info,turn_way);
}
...
static struct flashlight_ops smfled_impl_ops = {
	.set_movie_brightness = smfled_set_movie_brightness,
	.set_flash_brightness = smfled_set_flash_brightness,
...
static int smfled_check_hal_implement(struct sm_fled_hal *hal)
{
	...
	CHECK_HAL_IMPLEMENTED(fled_flash);
