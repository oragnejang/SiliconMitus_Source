...
static ssize_t flashlight_show_movie_max_brightness(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct flashlight_device *flashlight_dev = to_flashlight_device(dev);

	return sprintf(buf, "%d\n",
		       flashlight_dev->props.movie_max_brightness);
}

static ssize_t flashlight_show_flash_max_brightness(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct flashlight_device *flashlight_dev = to_flashlight_device(dev);

	return sprintf(buf, "%d\n",
		       flashlight_dev->props.flash_max_brightness);
}
...
static ssize_t flashlight_show_flash_delay(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct flashlight_device *flashlight_dev = to_flashlight_device(dev);

	return sprintf(buf, "%d\n",
		       flashlight_dev->props.flash_delay);
}

static ssize_t flashlight_store_movie_brightness(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (flashlight_dev->ops &&
	    flashlight_dev->ops->set_movie_brightness) {
		if (brightness > flashlight_dev->props.movie_max_brightness)
			rc = -EINVAL;
		else {
			pr_debug("flashlight: set movie brightness to %ld\n",
				 brightness);
			flashlight_dev->props.movie_brightness = brightness;
			flashlight_dev->ops->set_movie_brightness(
...
static ssize_t flashlight_show_movie_brightness(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct flashlight_device *flashlight_dev = to_flashlight_device(dev);

	return sprintf(buf, "%d\n",
		       flashlight_dev->props.movie_brightness);
}

static ssize_t flashlight_store_flash_brightness(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int rc;
	struct flashlight_device *flashlight_dev = to_flashlight_device(dev);
	...
	mutex_lock(&flashlight_dev->ops_lock);
	if (flashlight_dev->ops &&
	    flashlight_dev->ops->set_flash_brightness) {
		if (brightness > flashlight_dev->props.flash_max_brightness)
			rc = -EINVAL;
		else {
			pr_debug("flashlight: set flash brightness to %ld\n",
				 brightness);
			flashlight_dev->props.flash_brightness = brightness;
			flashlight_dev->ops->set_flash_brightness(
				flashlight_dev, brightness);
			rc = count;
		}
...
static ssize_t flashlight_show_flash_brightness(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct flashlight_device *flashlight_dev = to_flashlight_device(dev);

	return sprintf(buf, "%d\n",
		       flashlight_dev->props.flash_brightness);
}
...
static struct device_attribute flashlight_device_attributes[] = {
	...
	__ATTR(movie_max_brightness, 0444,
	flashlight_show_movie_max_brightness, NULL),
	__ATTR(flash_max_brightness, 0444,
	flashlight_show_flash_max_brightness, NULL),
	__ATTR(color_temperature, 0444,
	flashlight_show_color_temperature, NULL),
	__ATTR(flash_delay, 0444,
	flashlight_show_flash_delay, NULL),
	__ATTR(movie_brightness, 0644,
	flashlight_show_movie_brightness,
	flashlight_store_movie_brightness),
	__ATTR(flash_brightness, 0644,
	flashlight_show_flash_brightness,
	flashlight_store_flash_brightness),
	__ATTR_NULL,
};
...
int flashlight_set_movie_brightness(
	struct flashlight_device *flashlight_dev,
	int brightness_level)
{
	int rc;
	if ((flashlight_dev->ops ==  NULL) ||
	    (flashlight_dev->ops->set_movie_brightness == NULL))
		return -EINVAL;
	if (brightness_level > flashlight_dev->props.movie_max_brightness)
		return -EINVAL;
	mutex_lock(&flashlight_dev->ops_lock);
	rc = flashlight_dev->ops->set_movie_brightness(flashlight_dev,
			brightness_level);
	mutex_unlock(&flashlight_dev->ops_lock);
	if (rc < 0)
		return rc;
	flashlight_dev->props.movie_brightness = brightness_level;
	return rc;

}
EXPORT_SYMBOL(flashlight_set_movie_brightness);

int flashlight_set_flash_brightness(
	struct flashlight_device *flashlight_dev,
	int brightness_level)
{
	int rc;
	if ((flashlight_dev->ops ==  NULL) ||
	    (flashlight_dev->ops->set_flash_brightness == NULL))
		return -EINVAL;
	if (brightness_level > flashlight_dev->props.flash_max_brightness)
		return -EINVAL;
	mutex_lock(&flashlight_dev->ops_lock);
	rc = flashlight_dev->ops->set_flash_brightness(flashlight_dev,
			brightness_level);
	mutex_unlock(&flashlight_dev->ops_lock);
	if (rc < 0)
		return rc;
	flashlight_dev->props.flash_brightness = brightness_level;
	return rc;
}
EXPORT_SYMBOL(flashlight_set_flash_brightness);
...
int flashlight_flash(struct flashlight_device *flashlight_dev, int turn_way)
{
	if (flashlight_dev->ops == NULL ||
	    flashlight_dev->ops->flash == NULL)
		return -EINVAL;
	return flashlight_dev->ops->flash(flashlight_dev, turn_way);

	return -EINVAL;
}
EXPORT_SYMBOL(flashlight_flash);
...
int flashlight_flash_charge(struct flashlight_device *flashlight_dev,
			     flashlight_charge_event_cb cb, void *data, int start)
{

	if (flashlight_dev->ops->flash_charge)
		return flashlight_dev->ops->flash_charge(flashlight_dev,
				cb, data, start);
...
EXPORT_SYMBOL(flashlight_flash_charge);
