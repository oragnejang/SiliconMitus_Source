...
void sm5713_voffset_cancel(struct sm5713_fuelgauge_data *fuelgauge)
{
    ...
    {
        sm5713_write_word(fuelgauge->i2c, SM5713_FG_REG_VOLT_CAL, fuelgauge->info.volt_cal);
    }
    else
    {
...
static unsigned int sm5713_get_vbat(struct sm5713_fuelgauge_data *fuelgauge)
{
    ...
    if(sm5713_read_word(fuelgauge->i2c, SM5713_FG_REG_AUX_STAT) & 0x0002)
    {
        ...
        pr_info("%s : nENQ4 high, vsys register read result 0x%x\n", __func__, ret);
    }
		...
		if(fuelgauge->vempty_mode != VEMPTY_MODE_HW) 	
		{
				...
				(fuelgauge->info.batt_voltage < fuelgauge->info.n_tem_poff) && (!fuelgauge->is_charging))
			{
				...
				if((fuelgauge->info.p_batt_voltage <
					...
					(fuelgauge->info.n_tem_poff - fuelgauge->info.n_tem_poff_offset)))
				{
					sm5713_write_word(fuelgauge->i2c, SM5713_FG_REG_RS_MAN, fuelgauge->info.rs_value[0]>>1);
				}
				else
				{
					sm5713_write_word(fuelgauge->i2c, SM5713_FG_REG_RS_MAN, fuelgauge->info.rs_value[0]);
...
void sm5713_adabt_full_offset(struct sm5713_fuelgauge_data *fuelgauge)
{
    ...
    pr_info("%s: curr_off=%x, aux_stat=%x, \n", __func__, curr_off, aux_stat);

    if(((aux_stat & 0xD1) == 0x91) && (fuelgauge->info.batt_avgcurrent < 50))
    {
        fg_temp_gap = (fuelgauge->info.temp_fg/10) - fuelgauge->info.temp_std;
...
static bool sm5713_fg_reg_init(struct sm5713_fuelgauge_data *fuelgauge, bool is_surge)
{
	...
	for (i=0; i < TABLE_MAX; i++)
	{
		...
		for(j=0; j <= FG_TABLE_LEN; j++)
		{
			...
			msleep(10);
			if(fuelgauge->info.battery_table[i][j] != sm5713_fg_fs_read_word_table(fuelgauge->i2c, (table_reg + j), TABLE_READ_COUNT))
			{
	...
	// i ecv write
	sm5713_write_word(fuelgauge->i2c, SM5713_FG_REG_ECV_I_OFF, fuelgauge->info.ecv_i_off);
	sm5713_write_word(fuelgauge->i2c, SM5713_FG_REG_ECV_I_SLO, fuelgauge->info.ecv_i_slo);
	sm5713_write_word(fuelgauge->i2c, SM5713_FG_REG_DP_ECV_I_OFF, fuelgauge->info.dp_ecv_i_off);
	sm5713_write_word(fuelgauge->i2c, SM5713_FG_REG_DP_ECV_I_SLO, fuelgauge->info.dp_ecv_i_slo);
...
static void sm5713_fg_test_read(struct sm5713_fuelgauge_data *fuelgauge)
{
	...
	ret0 = sm5713_read_word(fuelgauge->i2c, 0xA0);
	ret1 = sm5713_read_word(fuelgauge->i2c, 0xAC);
	ret2 = sm5713_read_word(fuelgauge->i2c, 0xAD);
	ret3 = sm5713_read_word(fuelgauge->i2c, 0xAE);
	ret4 = sm5713_read_word(fuelgauge->i2c, 0xAF);
	...
	ret0 = sm5713_read_word(fuelgauge->i2c, 0xB0);
	ret1 = sm5713_read_word(fuelgauge->i2c, 0xBC);
	ret2 = sm5713_read_word(fuelgauge->i2c, 0xBD);
	ret3 = sm5713_read_word(fuelgauge->i2c, 0xBE);
	ret4 = sm5713_read_word(fuelgauge->i2c, 0xBF);
...
int sm5713_fg_get_jig_mode_real_vbat(struct sm5713_fuelgauge_data *fuelgauge, int meas_mode)
{
    int cntl, ret;
    ...
    if(meas_mode == 0)
    {
        cntl = sm5713_read_word(fuelgauge->i2c, SM5713_FG_REG_CNTL);
        pr_info("%s: start, CNTL=0x%x\n", __func__, cntl);
        
        cntl = cntl | ENABLE_MODE_nENQ4;
        sm5713_write_word(fuelgauge->i2c, SM5713_FG_REG_CNTL, cntl);
        msleep(300);
    }
    else
    {
...
#ifdef ENABLE_BATT_LONG_LIFE
    fuelgauge->info.v_max_now = sm5713_read_word(fuelgauge->i2c, 0xAE);
    pr_info("%s: v_max_now = 0x%x\n", __func__, fuelgauge->info.v_max_now);
    fuelgauge->info.v_max_now = sm5713_read_word(fuelgauge->i2c, 0xAE);
    pr_info("%s: v_max_now = 0x%x\n", __func__, fuelgauge->info.v_max_now);
    fuelgauge->info.q_max_now = sm5713_read_word(fuelgauge->i2c, 0xBE);
...
static int sm5713_fg_get_property(struct power_supply *psy,
                 ...
                 union power_supply_propval *val)
{
    struct sm5713_fuelgauge_data *fuelgauge = container_of(psy, struct sm5713_fuelgauge_data, psy_fg);
                 ...
                 const union power_supply_propval *val)
{
	...
	break;
    case POWER_SUPPLY_PROP_ENERGY_NOW:
        sm5713_fg_get_jig_mode_real_vbat(fuelgauge, val->intval);
        break;
    case POWER_SUPPLY_PROP_ENERGY_FULL_DESIGN:
...
static int sm5713_fuelgauge_probe(struct platform_device *pdev)
{
	...
	fuelgauge->psy_fg.name			= "sm5713-fuelgauge";
	fuelgauge->psy_fg.type			= POWER_SUPPLY_TYPE_UNKNOWN;
	fuelgauge->psy_fg.get_property	= sm5713_fg_get_property;
	fuelgauge->psy_fg.set_property	= sm5713_fg_set_property;
	fuelgauge->psy_fg.properties		= sm5713_fuelgauge_props;
	fuelgauge->psy_fg.num_properties	= ARRAY_SIZE(sm5713_fuelgauge_props);
