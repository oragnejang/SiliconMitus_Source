...
//#include <linux/ccic/pdic_notifier_sm.h>
...
void usbpd_manager_plug_attach(struct device *dev, muic_attached_dev_t new_dev)
{
	...
	sm5713_pdic_notifier_attach_attached_dev(manager->attached_dev);
#endif
}
...
void usbpd_manager_plug_detach(struct device *dev, bool notify)
{
	...
	if (notify)
		sm5713_pdic_notifier_detach_attached_dev(manager->attached_dev);
