...
static muic_data_t *afc_init_data;
...
EXPORT_SYMBOL(muic_disable_afc_state);

int muic_check_fled_state(bool enable, bool mode)
{
	muic_data_t *muic_data = afc_init_data;

	pr_info("[%s:%s] enable(%d), mode(%d)\n",MUIC_DEV_NAME, __func__, enable, mode);
	...
	pr_info("[%s:%s] fled_torch_enable(%d), fled_flash_enable(%d)\n",MUIC_DEV_NAME, __func__, muic_data->fled_torch_enable, muic_data->fled_flash_enable);

	if ( (muic_data->fled_torch_enable == false) && (muic_data->fled_flash_enable == false) ) {
		if ( (mode == 0) && (enable == 0)) {
			cancel_delayed_work(&muic_data->afc_torch_work);
			schedule_delayed_work(&muic_data->afc_torch_work, msecs_to_jiffies(5000)); 
		} else {
	...
	if (disable) { // AFC disable : 9V(12V) -> 5V
		if ((muic_data->attached_dev == ATTACHED_DEV_AFC_CHARGER_9V_MUIC) ||
			...
			if (muic_data->attached_dev == ATTACHED_DEV_QC_CHARGER_9V_MUIC){
				...
				sm5713_muic_voltage_control(SM5713_ENQC20_5V, SM5713_MUIC_QC20);
			} else {
				muic_data->old_afctxd = sm5713_i2c_read_byte(i2c, SM5713_MUIC_REG_AFCTXD);
				sm5713_muic_voltage_control(SM5713_MUIC_HV_5V, SM5713_MUIC_AFC_TA);
			}
		} else {
		...
		if ((muic_data->attached_dev == ATTACHED_DEV_AFC_CHARGER_5V_MUIC) ||
			...
			pr_info("[%s:%s] attached_dev(%d) , old_afctxd(0x%x)\n",MUIC_DEV_NAME, __func__, muic_data->attached_dev, muic_data->old_afctxd );
			
			if (muic_data->attached_dev == ATTACHED_DEV_QC_CHARGER_5V_MUIC){
				muic_data->old_afctxd = muic_data->qc20_vbus;
				sm5713_muic_voltage_control(SM5713_ENQC20_9V, SM5713_MUIC_QC20);
			} else {
				if ( muic_data->old_afctxd == 0x00 ) {
					sm5713_muic_voltage_control(SM5713_MUIC_HV_9V, SM5713_MUIC_AFC_TA);
				} else {
					sm5713_muic_voltage_control(muic_data->old_afctxd, SM5713_MUIC_AFC_TA);
				}
			}
		} else {
			pr_info("[%s:%s] skip: attached_dev(%d)\n",MUIC_DEV_NAME, __func__, muic_data->attached_dev );
...
int sm5713_muic_voltage_control(int afctxd, int qc20)
{
	...
	pr_info("[%s:%s] old_afctxd(0x%x), qc20(0x%x)\n", MUIC_DEV_NAME, __func__, muic_data->old_afctxd, qc20);

	muic_data->attached_dev = ATTACHED_DEV_AFC_CHARGER_PREPARE_MUIC;
	muic_notifier_attach_attached_dev(ATTACHED_DEV_AFC_CHARGER_PREPARE_MUIC);

	afc_prepare_state  = true;
	afc_prepare_afctxd = afctxd;
	afc_prepare_qc20   = qc20;

	cancel_delayed_work(&muic_data->afc_prepare_work);
	schedule_delayed_work(&muic_data->afc_prepare_work, msecs_to_jiffies(100)); // 100ms
	pr_info("[%s:%s] afc_prepare_work(afctxd=%d,qc20=%d) start  \n", MUIC_DEV_NAME, __func__, afc_prepare_afctxd, afc_prepare_qc20);
...
static void muic_afc_prepare_work(struct work_struct *work)
{
	...
	for (retry = 0; retry < 10; retry++) {
		psy_do_property("sm5713-charger", get, POWER_SUPPLY_PROP_CURRENT_MAX, value);	
		if (value.intval <= 500) {
	...
	if ( afc_prepare_qc20 == SM5713_MUIC_QC20) {
		...
		pr_info("[%s:%s] read REG_AFCCNTL=0x%x ,  write REG_AFCCNTL=0x%x , qc20_vbus=%d \n",
			MUIC_DEV_NAME, __func__, ret, reg_val, afc_prepare_afctxd );		
		...
		schedule_delayed_work(&muic_data->afc_retry_work, msecs_to_jiffies(5000)); // 5sec
		pr_info("[%s:%s] afc_retry_work(VBUS control) start \n", MUIC_DEV_NAME, __func__);

	    // ENAFC set '1'
	    sm5713_set_afc_ctrl_reg(muic_data, AFCCTRL_ENAFC, 1);
	}

	afc_prepare_state  = false;
	afc_prepare_afctxd = 0;
	afc_prepare_qc20   = 0;
...
int sm5713_afc_ta_attach(muic_data_t *muic_data)
{
	...
	if (ret < 0)
		printk(KERN_ERR "[%s:%s] err read VBUS %d\n", MUIC_DEV_NAME, __func__, ret);
	...
	if (ret < 0)
		printk(KERN_ERR "[%s:%s] err read AFC_STATUS %d\n", MUIC_DEV_NAME, __func__, ret);
	...
	if (ret < 0)
		printk(KERN_ERR "[%s:%s] err write AFC_TXD(%d)\n", MUIC_DEV_NAME, __func__, ret);
...
int sm5713_afc_vbus_update(muic_data_t *muic_data)
{
	...
	if (dev1 & DEV_TYPE1_QC20_TA){ // QC20_TA vbus update
		...
		pr_info("[%s:%s] QC20 vbus_voltage:%d mV (%d)\n"
				,MUIC_DEV_NAME, __func__,vbus_voltage, muic_data->qc20_vbus );
		...
		if ( (voltage_min <= vbus_voltage) && (vbus_voltage <= voltage_max) ) { /* AFC DONE */
			...
			if (muic_data->qc20_vbus == SM5713_ENQC20_12V){ // 12V retry
				if ( muic_data->afc_vbus_retry_count < 3 ) {
					...
					ret = sm5713_i2c_read_byte(i2c, SM5713_MUIC_REG_AFCCNTL);
					reg_val = (ret & 0x3F) | (SM5713_ENQC20_9V<<6);    // QC20 9V
					sm5713_i2c_write_byte(i2c, SM5713_MUIC_REG_AFCCNTL, reg_val);
					pr_info("[%s:%s] read REG_AFCCNTL=0x%x ,  write REG_AFCCNTL=0x%x , qc20_vbus=%d \n",
							MUIC_DEV_NAME, __func__, ret, reg_val, muic_data->qc20_vbus );
				...
				if ( muic_data->afc_vbus_retry_count < 3 ) {
					...
					muic_data->qc20_vbus = SM5713_ENQC20_5V;
					ret = sm5713_i2c_read_byte(i2c, SM5713_MUIC_REG_AFCCNTL);
					reg_val = (ret & 0x3F) | (SM5713_ENQC20_5V<<6);    // QC20 none
					sm5713_i2c_write_byte(i2c, SM5713_MUIC_REG_AFCCNTL, reg_val);
					pr_info("[%s:%s] read REG_AFCCNTL=0x%x ,  write REG_AFCCNTL=0x%x , qc20_vbus=%d \n",
						MUIC_DEV_NAME, __func__, ret, reg_val, muic_data->qc20_vbus );
		...
		vbus_txd_voltage = sm5713_i2c_read_byte(i2c, SM5713_MUIC_REG_AFCTXD);
		pr_info("[%s:%s] AFC_TXD [0x%02x]\n",MUIC_DEV_NAME, __func__, vbus_txd_voltage);
...
int sm5713_afc_multi_byte(muic_data_t *muic_data)
{
	...
	voltage_find = 0;
	for(i = 0 ; i < 15 ; i++ ){
		multi_byte[i] = sm5713_i2c_read_byte(i2c, SM5713_MUIC_REG_AFC_RXD1 + i);
		if (multi_byte[i] < 0) {
			printk(KERN_ERR "[%s:%s] err read AFC_RXD%d %d\n", 
	...
	ret = sm5713_i2c_write_byte(i2c, SM5713_MUIC_REG_AFCTXD, multi_byte[voltage_find]);
	if (ret < 0)
		printk(KERN_ERR "[%s:%s] err write AFC_TXD(%d)\n", MUIC_DEV_NAME, __func__, ret);
...
int sm5713_afc_error(muic_data_t *muic_data)
{
	...
	pr_info("[%s:%s] AFC_ERROR (%d) \n",MUIC_DEV_NAME, __func__, muic_data->afc_retry_count);
	...
	if (muic_data->afc_retry_count < 5) {
		dev1 = sm5713_i2c_read_byte(i2c, SM5713_MUIC_REG_DEVICETYPE1);
		pr_info("[%s:%s] DEVICE_TYPE1 [0x%02x]\n",MUIC_DEV_NAME, __func__, dev1);
		if ( (dev1 & DEV_TYPE1_QC20_TA) && (muic_data->afc_retry_count>=2) ){ // QC20_TA
			muic_data->qc20_vbus = SM5713_ENQC20_9V;
			ret = sm5713_i2c_read_byte(i2c, SM5713_MUIC_REG_AFCCNTL);
			reg_val = (ret & 0x3F) | (SM5713_ENQC20_9V<<6);    // QC20 9V
			sm5713_i2c_write_byte(i2c, SM5713_MUIC_REG_AFCCNTL, reg_val);
...
int sm5713_set_afc_ctrl_reg(muic_data_t *muic_data, int shift, bool on)
{
    ...
    pr_info("[%s:%s] Register[%d], set [%d]\n", MUIC_DEV_NAME, __func__, shift, on);
    ret = sm5713_i2c_read_byte(i2c, SM5713_MUIC_REG_AFCCNTL);
    if (ret < 0)
    ...
    if (reg_val ^ ret) {
        printk(KERN_DEBUG "[%s:%s] reg_val(0x%x) != AFC_CTRL reg(0x%x), update reg\n",
            MUIC_DEV_NAME, __func__, reg_val, ret);

        ret = sm5713_i2c_write_byte(i2c, SM5713_MUIC_REG_AFCCNTL,
                reg_val);
        if (ret < 0)
            printk(KERN_ERR "[%s:%s] err write AFC_CTRL(%d)\n",
                    MUIC_DEV_NAME, __func__, ret);
    } else {
        printk(KERN_DEBUG "[%s:%s] (0x%x), just return\n",
                MUIC_DEV_NAME, __func__, ret);
        return 0;
    }
    ...
    ret = sm5713_i2c_read_byte(i2c, SM5713_MUIC_REG_AFCCNTL);
    if (ret < 0)
        printk(KERN_ERR "[%s:%s] err read AFC_CTRL(%d)\n",
            MUIC_DEV_NAME, __func__, ret);
    else
        printk(KERN_DEBUG "[%s:%s] AFC_CTRL reg after change(0x%x)\n",
            MUIC_DEV_NAME, __func__, ret);
...
void sm5713_hv_muic_init_detect(muic_data_t *muic_data)
{
	int afc_ta_attached = 0;

	pr_info("[%s:%s] \n", MUIC_DEV_NAME, __func__);

	afc_ta_attached = sm5713_i2c_read_byte(muic_data->i2c, SM5713_MUIC_REG_AFCTASTATUS);
...
static void muic_focrced_detection_by_charger(struct work_struct *work)
{
	...
	sm5713_hv_muic_init_detect(muic_data);
	
	return;
}

void sm5713_muic_charger_init(void)
{
	pr_info("[%s:%s]\n", MUIC_DEV_NAME, __func__);
