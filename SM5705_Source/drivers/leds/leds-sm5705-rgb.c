...
static char sm5705_led_str[3][10] = {"led_r", "led_g", "led_b"};
...
static unsigned char calc_delaytime_offset_to_ms(struct sm5705_rgb *sm5705_rgb, unsigned int ms)
{
    ...
    if (ms > 7500) {
        dev_info(sm5705_rgb->dev, "SM5705 delay time limit = 7.5sec, correct input value (ms=%d)\n", ms);
        time = 0xF;
    } else {
        time = ms / 500;
    }
...
static unsigned char calc_steptime_offset_to_ms(struct sm5705_rgb *sm5705_rgb, unsigned int ms)
{
    ...
    if (ms > 60) {
        dev_info(sm5705_rgb->dev, "SM5705 step time limit = 60ms, correct input value (ms=%d)\n", ms);
        time = 0xF;
    } else {
        time = ms / 4;
    }
...
static ssize_t store_led_r(struct device *dev, struct device_attribute *devattr, const char *buf, size_t count)
{
    ...
    dev_info(dev, "%s:current=0x%x, constant %s\n", __func__, brightness, (brightness) ? "ON" : "OFF");
...
static ssize_t store_sm5705_rgb_blink(struct device *dev, struct device_attribute *devattr, const char *buf, size_t count)
{
    ...
    for (i=0; i < 3; ++i) {
        if (led_br[i]) {
            ...
            sm5705_rgb_set_LEDx_slopemode(sm5705_rgb, i, 0, sm5705_rgb->delay_on_times_ms, sm5705_rgb->delay_off_times_ms, 32, 32);
            sm5705_rgb_set_LEDx_current(sm5705_rgb, i, led_current);
            sm5705_rgb_set_LEDx_enable(sm5705_rgb, i, 1, 1);
...
static DEVICE_ATTR(led_brightness, 0660, NULL, store_sm5705_rgb_brightness);
static DEVICE_ATTR(led_lowpower, 0660, NULL,  store_sm5705_rgb_lowpower);
...
static int get_rgb_index_from_led_cdev(struct sm5705_rgb *sm5705_rgb, struct led_classdev *led_cdev)
{
    ...
    if (sm5705_rgb == NULL || led_cdev == NULL) {
        dev_err(sm5705_rgb->dev, "%s: invalid parameter (sm5705_rgb=%p, led_cdev=%p)\n", __func__, sm5705_rgb, led_cdev);
        return -EINVAL;
    }
...
static void sm5705_rgb_brightness_set(struct led_classdev *led_cdev, unsigned int brightness)
{
    ...
    dev_info(dev, "%s: led[%d] current=0x%x, constant %s\n", __func__, index, brightness, (brightness) ? "ON" : "OFF");
}
...
static unsigned int sm5705_rgb_brightness_get(struct led_classdev *led_cdev)
{
    ...
    dev_info(dev, "%s: led[%d] get current : 0x%x\n", __func__, index, cur_value);
...
static inline void _decide_octa(char *octa, unsigned char octa_color)
{
    ...
    case 4:
        strcpy(octa, "_gr");    break;
    case 5:
        strcpy(octa, "_rd");    break;
    default:
        break;
    }
}
...
static int sm5705_rgb_parse_dt(struct device *dev, unsigned char octa_color, char **leds_name)
{
    ...
    dev_info(dev, "%s: Device Tree Parse done.\n", __func__);
...
static inline int _register_led_commonclass_dev(struct device *dev, struct sm5705_rgb *sm5705_rgb, char **leds_name)
{
    ...
    for (i=0; i < LED_MAX; ++i) {
        ...
        dev_info(dev, "%s: before led[%d].name = %s / leds[%d]_name = %s\n", __func__, i, sm5705_rgb->led[i].name, i, leds_name[i]);
        sm5705_rgb->led[i].name = sm5705_led_str[i];
        dev_info(dev, "%s: before led[%d].name = %s / sm5705_led_str[%d] = %s\n", __func__, i, sm5705_rgb->led[i].name, i, sm5705_led_str[i]);
    }
    ...
    for (i=0; i < LED_MAX; ++i) {
        ...
        dev_info(dev, "%s: register led[%d] commonclass_dev : .name=%s, .max_brightness=%d, .brightness_set=%p, ./brightness_get=%p\n", __func__, i, sm5705_rgb->led[i].name, sm5705_rgb->led[i].max_brightness, sm5705_rgb->led[i].brightness_set, sm5705_rgb->led[i].brightness_get);
        if (!sm5705_rgb->led[i].name || !sm5705_rgb->led[i].brightness_set || !sm5705_rgb->led[i].brightness_get) {
            return -EINVAL;
        }
...
static int sm5705_rgb_probe(struct platform_device *pdev)
{
	...
	unsigned char octa_color = (lcdtype >> 16) & 0xf;
...
static struct platform_driver sm5705_rgbled_driver = {
	...
	.remove		= __devexit_p(sm5705_rgb_remove),
};
