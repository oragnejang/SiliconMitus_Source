...
static int sm5705_fled_initialize(struct sm5705_fled_info *sm5705_fled)
{
    ...
    for (i=0; i < SM5705_FLED_MAX; ++i) {
        if (pdata->led[i].used_gpio) {
            ...
            ret = sm5705_FLEDx_mode_enable(sm5705_fled, i, SM5705_FLED_ON_EXTERNAL_CONTROL_MODE);
            if (IS_ERR_VALUE(ret)) {
...
#ifdef CONFIG_OF
static int sm5705_fled_parse_dt(struct device *dev, struct sm5705_fled_platform_data *pdata)
{
...
static int sm5705_fled_probe(struct platform_device *pdev)
{
    ...
    dev_info(dev, "SM5705 Flash-LED devic driver Probing..\n");
