...
static int __devinit sec_battery_probe(struct platform_device *pdev)
{
	...
	pr_info("%s : CHARGER CHIP NAME (%s)\n", __func__, charger_chip_name);
	if (charger_chip_name != NULL && !strcmp(charger_chip_name,"sm5705-charger")) {
		battery->pdata->charger_name = "sm5705-charger";
		battery->pdata->fuelgauge_name = "sm5705-fuelgauge";
    }
#endif
