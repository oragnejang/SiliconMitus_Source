...
static __devinit int sm5705_pmic_probe(struct platform_device *pdev)
{
    struct sm5705_dev *iodev = dev_get_drvdata(pdev->dev.parent);
...
static int __devexit sm5705_pmic_remove(struct platform_device *pdev)
{
...
static struct platform_driver sm5705_pmic_driver = {
    ...
    .remove = __devexit_p(sm5705_pmic_remove),
