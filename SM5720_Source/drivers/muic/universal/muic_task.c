...
#if defined(CONFIG_MUIC_UNIVERSAL_SM5720_AFC)
...
extern void sm5720_muic_check_reset(struct i2c_client *i2c, void *muic_data );
...
static int muic_irq_init(muic_data_t *pmuic)
{
	...
	if(!strcmp(pmuic->chip_name,"max,max77854")) {
		...
		irq_int2_rid_charger = pdata->irq_gpio + SM5720_MUIC_IRQ_INT2_RID_CHARGER;
...
{
#if defined(CONFIG_MUIC_UNIVERSAL_SM5720)
	struct sm5720_dev *pmfd = dev_get_drvdata(pdev->dev.parent);
	struct sm5720_platform_data *mfd_pdata = dev_get_platdata(pmfd->dev);
#else
	...
	pr_info("%s: fuel i2c client %s at 0x%02x\n", __func__, pmfd->fuelgauge->name, pmfd->fuelgauge->addr);
	pr_info("%s: chg  i2c client %s at 0x%02x\n", __func__, pmfd->charger->name, pmfd->charger->addr);
#else
	pr_info("%s:  muic i2c client %s at 0x%02x\n", __func__, pmfd->muic->name, pmfd->muic->addr);
...
#if defined(CONFIG_MUIC_UNIVERSAL_SM5720)
	sm5720_muic_init_afc_state(pmuic);
    pmfd->check_muic_reset = sm5720_muic_check_reset;
