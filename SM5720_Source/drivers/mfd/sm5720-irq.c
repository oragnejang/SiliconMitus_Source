...
static const struct sm5720_irq_data sm5720_irqs[] = {
    /* MUIC-irqs */
    DECLARE_IRQ(SM5720_MUIC_IRQ_INT1_LKR,	            MUIC_INT1,      1 << 4),    
    DECLARE_IRQ(SM5720_MUIC_IRQ_INT1_LKP,	            MUIC_INT1,      1 << 3),    
    DECLARE_IRQ(SM5720_MUIC_IRQ_INT1_KP,	            MUIC_INT1,      1 << 2),
    ...
    DECLARE_IRQ(SM5720_MUIC_IRQ_INT2_RID_CHARGER,       MUIC_INT2,      1 << 6),
    DECLARE_IRQ(SM5720_MUIC_IRQ_INT2_MHL,               MUIC_INT2,      1 << 5),
    DECLARE_IRQ(SM5720_MUIC_IRQ_INT2_STUCK_KEY_RCV,	    MUIC_INT2,      1 << 4),
    DECLARE_IRQ(SM5720_MUIC_IRQ_INT2_STUCK_KEY,         MUIC_INT2,      1 << 3),
    DECLARE_IRQ(SM5720_MUIC_IRQ_INT2_ADC_CHG,           MUIC_INT2,      1 << 2),
    DECLARE_IRQ(SM5720_MUIC_IRQ_INT2_REV_ACCE,          MUIC_INT2,      1 << 1),
    ...
    DECLARE_IRQ(SM5720_CHG_IRQ_INT3_THEMREG,	        CHG_INT3,       1 << 0),
    DECLARE_IRQ(SM5720_CHG_IRQ_INT3_THEMSHDN,	        CHG_INT3,       1 << 1),
    ...
    DECLARE_IRQ(SM5720_CHG_IRQ_INT4_BOOSTPOK_NG,	    CHG_INT4,       1 << 0),
    DECLARE_IRQ(SM5720_CHG_IRQ_INT4_BOOSTPOK,	        CHG_INT4,       1 << 1),
    DECLARE_IRQ(SM5720_CHG_IRQ_INT4_THEM_H,	            CHG_INT4,       1 << 2),
    DECLARE_IRQ(SM5720_CHG_IRQ_INT4_THEM_C,	            CHG_INT4,       1 << 3),
    DECLARE_IRQ(SM5720_CHG_IRQ_INT4_nVBUSOK,	        CHG_INT4,       1 << 4),
    DECLARE_IRQ(SM5720_CHG_IRQ_INT4_nWPCOK,	            CHG_INT4,       1 << 5),
    DECLARE_IRQ(SM5720_CHG_IRQ_INT4_MRSTB,              CHG_INT4,       1 << 6),
    DECLARE_IRQ(SM5720_CHG_IRQ_INT4_WPCIN_UPDATE,       CHG_INT4,       1 << 7),

    /* FuelGauge-irqs */
    DECLARE_IRQ(SM5720_FG_IRQ_INT_LOW_VOLTAGE,          FG_INT,         1 << 0),
    DECLARE_IRQ(SM5720_FG_IRQ_INT_LOW_TEMP,	            FG_INT,         1 << 1),
    DECLARE_IRQ(SM5720_FG_IRQ_INT_HIGH_TEMP,	        FG_INT,         1 << 2),
...
static irqreturn_t sm5720_irq_thread(int irq, void *data)
{
	...
	pr_debug("%s: irq gpio pre-state(0x%02x)\n", __func__, gpio_get_value(sm5720->irq_gpio));
...
int sm5720_irq_init(struct sm5720_dev *sm5720)
{
    ...
    sm5720->irq = gpio_to_irq(sm5720->irq_gpio);
    pr_info("%s:%s irq=%d, irq->gpio=%d\n", MFD_DEV_NAME, __func__, sm5720->irq, sm5720->irq_gpio);

    ret = gpio_request(sm5720->irq_gpio, "if_pmic_irq");
    if (ret) {
        dev_err(sm5720->dev, "%s: fail requesting gpio %d\n", __func__, sm5720->irq_gpio);
        return ret;
    }
    gpio_direction_input(sm5720->irq_gpio);
    gpio_free(sm5720->irq_gpio);
    ...
    for (i = 0; i < SM5720_IRQ_GROUP_NR; i++) {
        ...
        if (i == FG_INT) {
            sm5720_write_word(i2c, sm5720_mask_reg[i], 0xff);
        } else {
            sm5720_write_reg(i2c, sm5720_mask_reg[i], 0xff);
        }
    }
