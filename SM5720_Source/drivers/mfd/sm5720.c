...
static int of_sm5720_dt(struct device *dev, struct sm5720_platform_data *pdata)
{
	...
	pdata->irq_gpio = of_get_named_gpio(np_sm5720, "sm5720, irq-gpio", 0);
...
static int sm5720_i2c_pinctrl_init(struct sm5720_dev *sm5720) {
	...
	sm5720->i2c_pinctrl = devm_pinctrl_get(&sm5720->charger->dev);
	...
	sm5720->i2c_gpio_state_active = pinctrl_lookup_state(i2c_pinctrl, "sm5720_active");
	...
	sm5720->i2c_gpio_state_suspend = pinctrl_lookup_state(i2c_pinctrl, "sm5720_suspend");
...
static int sm5720_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *dev_id)
{
	...
	if (pdata) {
		...
		sm5720->irq_gpio = pdata->irq_gpio;
    ...
    /* Check vender ID */
	if (sm5720_read_reg(i2c, SM5720_CHG_REG_DEVICEID, &reg_data) < 0) {
