...
#include <linux/battery/fuelgauge/sm5720_fuelgauge.h>
...
#ifdef BAT_R_SUPP
	curr = sm5720_get_curr(fuelgauge);
#endif
	if (ret<0) {
...
#ifdef BAT_R_SUPP
	pr_info("%s: BAT_R_SUPP result curr : vbat , (%d) :(%d)\n", __func__, curr, vbat);
	vbat = vbat - ((curr*BAT_R_SUPP)/1000);
#endif
