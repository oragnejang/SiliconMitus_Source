...
static void sm5702_CHG_print_reg(struct sm5720_charger_data *charger)
{
...
static int psy_chg_set_cable_online(struct sm5720_charger_data *charger, int cable_type)
{
    ...
    if (charger->cable_type == POWER_SUPPLY_TYPE_POWER_SHARING) {
        ...
        sm5720_charger_oper_push_event(SM5720_CHARGER_OP_EVENT_PWR_SHAR, value.intval);
        ...
        sm5720_chg_set_input_limit(charger, charger->input_current);
...
static irqreturn_t chg_aicl_isr(int irq, void *data)
{
    ...
    queue_delayed_work_on(0, charger->wqueue, &charger->aicl_work, 0);
...
{
	struct sm5720_charger_data *charger =
		container_of(work, struct sm5720_charger_data, afc_work.work);
...
static int sm5720_charger_parse_dt(struct sm5720_charger_data *charger)
{
	...
	if (!np) {
		...
		pdata->charging_current = kzalloc(sizeof(sec_charging_current_t) * len, GFP_KERNEL);
...
static void sm5720_charger_initialize(struct sm5720_charger_data *charger)
{
	...
	sm5720_CHG_set_BATREG(charger, charger->pdata->chg_float_voltage);
