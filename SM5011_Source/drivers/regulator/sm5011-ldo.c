...
static struct sm5011_ldo_info *static_info = NULL;

struct sm5011_ldo_info {	
	struct device			*dev;
	struct sm5011_pmic_dev *iodev;
	struct regulator_dev *rdev[SM5011_LDOMAX];
...
static int _sm5011_ldo_is_enabled(struct regulator_dev *rdev)
{
	struct sm5011_ldo_info *sm5011 = rdev_get_drvdata(rdev);
	...
	entcxo_pin_sta = gpio_get_value(sm5011->entcxo_pin);

	/* Check ENL16 Pin Condtion */
	enl16_pin_sta = gpio_get_value(sm5011->enl16_pin);
	...
	switch (reg_id) {			
		case SM5011_LDO1 ... SM5011_LDO5:
		case SM5011_LDO7 ... SM5011_LDO15:			
		case SM5011_LDO17 ... SM5011_LDO20:						
				     ...
				     unsigned int mode)
{
	...
	if (reg_id < SM5011_LDO1 && reg_id > SM5011_LDO20) {		
	...
	/* 
	  ...
	  * register is set to SM5011_LPM_OPMODE_nPWRSTM 
	  */
	if (enpwrstm_sta == 1 && pdata->ldos[reg_id].always_lpm_npwrstm == 1)
	{
		pr_info("%s: [%s] Controlled by nPWRSTM\n", __func__, rdev->desc->name);
		val = SM5011_LPM_OPMODE_nPWRSTM << SM5011_LPM_OPMODE_SHIFT;
		return sm5011_reg_update(sm5011->iodev, rdev->desc->vsel_reg,
				  val, SM5011_LPM_OPMODE_MASK);
	}

	/* Non SM5011_LPM_OPMODE_nPWRSTM */
	switch (mode) {
	case REGULATOR_MODE_STANDBY:
		val = SM5011_LPM_OPMODE_ON << SM5011_LPM_OPMODE_SHIFT;
		break;
	case REGULATOR_MODE_NORMAL:
		val = SM5011_LPM_OPMODE_OFF << SM5011_LPM_OPMODE_SHIFT;
		break;		
	default:
...
static unsigned int sm5011_get_mode(struct regulator_dev *rdev)
{
	...
	enabled_val = _sm5011_ldo_is_enabled(rdev);

	ret = sm5011_reg_read(sm5011->iodev, rdev->desc->vsel_reg, &val);
	if (ret)
		return ret;
	enlpm_sta = ((val & SM5011_LPM_OPMODE_MASK) >> SM5011_LPM_OPMODE_SHIFT);
	...
	/* 
	  ...
	  */
	if (enpwrstm_sta == 1 && pdata->ldos[reg_id].always_lpm_npwrstm == 1 
		&& enabled_val == 1 && enlpm_sta == SM5011_LPM_OPMODE_nPWRSTM)
	{	
		pr_info("%s: [%s] Controlled by nPWRSTM\n", __func__, rdev->desc->name);
	...
	/* Non SM5011_LPM_OPMODE_nPWRSTM */
	return enlpm_sta == SM5011_LPM_OPMODE_ON ? REGULATOR_MODE_STANDBY : REGULATOR_MODE_NORMAL;
}
...
static int sm5011_enable(struct regulator_dev *rdev)
{
	...
	switch (reg_id)
	{
		case SM5011_LDO1 ... SM5011_LDO20:
			/* 
			  * ENnPWRSTM Bit is 1 and ldo is always controlled by nPWRSTM , 
			  * register is set to SM5011_OPMODE_nPWRSTM 
			  */
...
{
	return _sm5011_ldo_is_enabled(rdev);
}
...
#define _LDO(macro)	SM5011_LDO##macro
#define _ldo_ops(num)	sm5011_ldo_ops##num
#define _REG_LDOxCNTLx(num1, num2)	SM5011_REG_LDO##num1##CNTL##num2

#define _REG(ctrl)	SM5011_REG##ctrl

#define SM5011_LDO_DESC(_name, _id, _ops, _mu, _us, _nv, _vr, _vm, _er, _em)	{	\
	.name		= _name,				\
	.id			= _id,					\
	.ops		= _ops,					\
...
static struct regulator_desc sm5011_lods_desc[SM5011_LDOMAX] = {
	/* name, id, ops, min_uv, uV_step, n_voltages, vsel_reg, vsel_mask, enable_reg, enable_mask */
	SM5011_LDO_DESC("LDO1", _LDO(1), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51),
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(1,2), _REG_LDOxCNTLx(1,1), _LDO(_ENABLE_MASK_03)),
	SM5011_LDO_DESC("LDO2", _LDO(2), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51), 
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(2,2), _REG_LDOxCNTLx(2,1), _LDO(_ENABLE_MASK_03)),	
	SM5011_LDO_DESC("LDO3", _LDO(3), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51), 
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(3,2), _REG_LDOxCNTLx(3,1), _LDO(_ENABLE_MASK_03)),	
	SM5011_LDO_DESC("LDO4", _LDO(4), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51), 
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(4,2), _REG_LDOxCNTLx(4,1), _LDO(_ENABLE_MASK_03)),						
	SM5011_LDO_DESC("LDO5", _LDO(5), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51), 
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(5,2), _REG_LDOxCNTLx(5,1), _LDO(_ENABLE_MASK_03)),
	SM5011_LDO_DESC("LDO6", _LDO(6), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51), 
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(6,2), _REG_LDOxCNTLx(6,1), _LDO(_ENABLE_MASK_03)),	
	SM5011_LDO_DESC("LDO7", _LDO(7), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51), 
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(7,2), _REG_LDOxCNTLx(7,1), _LDO(_ENABLE_MASK_03)),	
	SM5011_LDO_DESC("LDO8", _LDO(8), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51), 
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(8,2), _REG_LDOxCNTLx(8,1), _LDO(_ENABLE_MASK_03)),
	SM5011_LDO_DESC("LDO9", _LDO(9), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51), 
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(9,2), _REG_LDOxCNTLx(9,1), _LDO(_ENABLE_MASK_03)),
	SM5011_LDO_DESC("LDO10", _LDO(10), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51), 
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(10,2), _REG_LDOxCNTLx(10,1), _LDO(_ENABLE_MASK_03)),	
	SM5011_LDO_DESC("LDO11", _LDO(11), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51), 
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(11,2), _REG_LDOxCNTLx(11,1), _LDO(_ENABLE_MASK_03)),	
	SM5011_LDO_DESC("LDO12", _LDO(12), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51), 
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(12,2), _REG_LDOxCNTLx(12,1), _LDO(_ENABLE_MASK_03)),	
	SM5011_LDO_DESC("LDO13", _LDO(13), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51), 
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(13,2), _REG_LDOxCNTLx(13,1), _LDO(_ENABLE_MASK_03)),
	SM5011_LDO_DESC("LDO14", _LDO(14), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51), 
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(14,2), _REG_LDOxCNTLx(14,1), _LDO(_ENABLE_MASK_03)),	
	SM5011_LDO_DESC("LDO15", _LDO(15), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51), 
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(15,2), _REG_LDOxCNTLx(15,1), _LDO(_ENABLE_MASK_03)),	
	SM5011_LDO_DESC("LDO16", _LDO(16), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51), 
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(16,2), _REG_LDOxCNTLx(16,1), _LDO(_ENABLE_MASK_03)),						
	SM5011_LDO_DESC("LDO17", _LDO(17), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51), 
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(17,2), _REG_LDOxCNTLx(17,1), _LDO(_ENABLE_MASK_03)), 					
	SM5011_LDO_DESC("LDO18", _LDO(18), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51), 
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(18,2), _REG_LDOxCNTLx(18,1), _LDO(_ENABLE_MASK_03)), 					
	SM5011_LDO_DESC("LDO19", _LDO(19), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51), 
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(19,2), _REG_LDOxCNTLx(19,1), _LDO(_ENABLE_MASK_03)), 					
	SM5011_LDO_DESC("LDO20", _LDO(20), &_ldo_ops(), _LDO(_MIN_0P8V), _LDO(_STEP_50P0MV), _LDO(_NUM_51), 
					_LDO(_VSEL_MASK_3F), _REG_LDOxCNTLx(20,2), _REG_LDOxCNTLx(20,1), _LDO(_ENABLE_MASK_03)), 										
};

#ifdef CONFIG_OF
static int sm5011_ldo_dt_parse_pdata(struct sm5011_pmic_dev *iodev,
					struct sm5011_platform_data *pdata)
{
	...
	ldo_np = of_find_node_by_name(pmic_np, "sm5011-ldo");
	...
	for_each_child_of_node(ldos_np, reg_np) {
		for (i = 0; i < ARRAY_SIZE(sm5011_lods_desc); i++)
			if (!of_node_cmp(reg_np->name, sm5011_lods_desc[i].name))
				break;

		if (i == ARRAY_SIZE(sm5011_lods_desc)) {
		...
		rdata->initdata = of_get_regulator_init_data(
						iodev->dev, reg_np, &sm5011_lods_desc[i]);
...
static int sm5011_ldo_preinit(struct sm5011_ldo_info *info)
{
	int ret = 0;
...
static int sm5011_ldo_probe(struct platform_device *pdev)
{
	struct sm5011_pmic_dev *iodev = dev_get_drvdata(pdev->dev.parent);
	struct sm5011_platform_data *pdata = iodev->pdata;
	struct regulator_config config = { };
	struct sm5011_ldo_info *info;
	int i;
	int ret = 0;

	pr_info("%s : Start\n", __func__);

	info = devm_kzalloc(&pdev->dev, sizeof(struct sm5011_ldo_info),
				GFP_KERNEL);
	if (!info)
		return -ENOMEM;
	...
	if (iodev->dev->of_node) {
		ret = sm5011_ldo_dt_parse_pdata(iodev, pdata);
		if (ret) {
			return ret;
		}
	}
	...
	for (i = 0; i < pdata->num_ldos; i++) {
		...
		info->rdev[i] = regulator_register(&sm5011_lods_desc[id], &config);
	...
	return 0;
err:
	for (i = 0; i < SM5011_LDOMAX; i++)
		regulator_unregister(info->rdev[i]);

	return ret;
}

static int sm5011_ldo_remove(struct platform_device *pdev)
{
	struct sm5011_ldo_info *info = platform_get_drvdata(pdev);
	int i;
...
static const struct of_device_id sm5011_ldo_of_match[] = {
	{ .compatible = "sm,sm5011-ldo", },
	{ },
};
MODULE_DEVICE_TABLE(platform, sm5011_ldo_of_match);

static const struct platform_device_id sm5011_ldo_id[] = {
	{ "sm5011-ldo", 0 },
};

static struct platform_driver sm5011_ldo_driver = {
	.driver = {
		.name = "sm5011-ldo",
		.owner = THIS_MODULE,
		.of_match_table = sm5011_ldo_of_match,		
	},
	.probe = sm5011_ldo_probe,
	.remove = sm5011_ldo_remove,
	.id_table = sm5011_ldo_id,
};

static int __init sm5011_ldo_init(void)
{
	pr_info("%s\n", __func__);

	return platform_driver_register(&sm5011_ldo_driver);
}
module_init(sm5011_ldo_init);

static void __exit sm5011_regulator_exit(void)
{
	platform_driver_unregister(&sm5011_ldo_driver);
}
module_exit(sm5011_regulator_exit);

/* Module information */
MODULE_DESCRIPTION("SM5011 Regulator LDO Driver");
MODULE_LICENSE("GPL");
