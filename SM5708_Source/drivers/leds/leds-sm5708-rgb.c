...
static int sm5708_rgb_set_LEDx_slopeduty(struct sm5708_rgb *sm5708_rgb, int index, unsigned char max_duty, unsigned char mid_duty, unsigned char min_duty)
{
	unsigned char reg_val;
	int ret;
...
static int sm5708_rgb_set_LEDx_slopemode(struct sm5708_rgb *sm5708_rgb, u32 index, u32 delay_ms, u32 delay_on_time_ms, u32 delay_off_time_ms, u32 step_on_time_ms, u32 step_off_time_ms)
{
...
static ssize_t store_sm5708_rgb_brightness(struct device *dev, struct device_attribute *devattr, const char *buf, size_t count)
{
	struct sm5708_rgb *sm5708_rgb = dev_get_drvdata(dev);
	u8 brightness;
	int ret;
...
static ssize_t store_sm5708_rgb_lowpower(struct device *dev, struct device_attribute *devattr, const char *buf, size_t count)
{
	struct sm5708_rgb *sm5708_rgb = dev_get_drvdata(dev);
	int ret;
...
static unsigned char calc_led_current_from_lowpower_brightness(struct sm5708_rgb *sm5708_rgb, int index, unsigned char brightness)
{
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dev_info(dev, "%s: delay_on_time:%d, delay_off_time:%d, current:0x%x\n", __func__, sm5708_rgb->delay_on_times_ms, sm5708_rgb->delay_off_times_ms, normal_powermode_current);
