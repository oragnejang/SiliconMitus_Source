/*
*  /drivers/battery/sm5708_charger_1.c
*
...
static bool sm5708_CHG_get_INT_STATUS(struct sm5708_charger_data *charger,	unsigned char index, unsigned char offset)
{
...
{
	sm5708_update_reg(charger->i2c, SM5708_REG_CHGCNTL7, ((topoff_timer & 0x3) << 3), (0x3 << 3));
...
static int sm5708_CHG_set_FASTCHG(struct sm5708_charger_data *charger, unsigned short FASTCHG_mA)
{
...
static bool sm5708_charger_get_discharging_status(struct sm5708_charger_data *charger)
{
	unsigned char reg;

	reg = sm5708_CHG_read_reg(charger, SM5708_REG_FACTORY);
	pr_info("enable:(%s), reg[0x%02X]:0x%02x\n", (reg & SM5708_EN_DISCHG_FORCE_MASK) ? "ON" : "OFF" , SM5708_REG_FACTORY, reg);

	if (reg & SM5708_EN_DISCHG_FORCE_MASK)
		return true;
	else
		return false;
}

static void sm5708_charger_en_discharging_force(struct sm5708_charger_data *charger, bool enable)
{
	unsigned char reg;

	if (enable)
		sm5708_update_reg(charger->i2c, SM5708_REG_FACTORY,	SM5708_EN_DISCHG_FORCE_MASK, SM5708_EN_DISCHG_FORCE_MASK);
	else
		sm5708_update_reg(charger->i2c, SM5708_REG_FACTORY,	0, SM5708_EN_DISCHG_FORCE_MASK);

	reg = sm5708_CHG_read_reg(charger, SM5708_REG_FACTORY);
	pr_info("enable(%s), reg[0x%02X]:0x%02x\n",	enable ? "ON" : "OFF", SM5708_REG_FACTORY, reg);
}
#endif
...
static void psy_chg_set_cable_online(struct sm5708_charger_data *charger, int cable_type)
{
	...
	if (charger->cable_type == POWER_SUPPLY_TYPE_POWER_SHARING) {
		...
		sm5708_charger_oper_push_event(SM5708_CHARGER_OP_EVENT_OTG, 1);
				...
				enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		...
		if (charger->charging_current_max != val->intval) {
			pr_info("change input current max (%d --> %d)\n", charger->charging_current_max, val->intval);
			charger->charging_current_max = val->intval;
		}
		sm5708_set_input_current(charger, val->intval);
		break;
	case POWER_SUPPLY_PROP_CURRENT_AVG:
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		pr_info("POWER_SUPPLY_PROP_CURRENT_AVG/NOW - current=%d\n", val->intval);
		charger->charging_current = val->intval;
		sm5708_set_charge_current(charger, val->intval);
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		...
		if (buck_state) {
			...
			pr_info("update op_mode : SM5708_CHARGER_OP_MODE_CHG_ON\n");
		} else {
			sm5708_update_reg(charger->i2c, SM5708_REG_CNTL, SM5708_CHARGER_OP_MODE_SUSPEND, 0x07);
			pr_info("update op_mode : SM5708_CHARGER_OP_MODE_SUSPEND\n");
		}
		break;
#if defined(CONFIG_BATTERY_SWELLING_SELF_DISCHARGING)
	case POWER_SUPPLY_PROP_RESISTANCE:
		sm5708_charger_en_discharging_force(charger, val->intval);
		break;
#endif
...
static int sm5708_chg_get_property(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
	struct sm5708_charger_data *charger = container_of(psy, struct sm5708_charger_data, psy_chg);
...
#if defined(CONFIG_BATTERY_SWELLING_SELF_DISCHARGING)
	case POWER_SUPPLY_PROP_RESISTANCE:
		val->intval = sm5708_charger_get_discharging_status(charger);
		break;
#endif
	default:
		pr_err("un-known Power-supply property type (psp=%d)\n", psp);
...
static int sm5708_otg_set_property(struct power_supply *psy, enum power_supply_property psp, const union power_supply_propval *val)
{
	struct sm5708_charger_data *charger = container_of(psy, struct sm5708_charger_data, psy_otg);
	union power_supply_propval value;
...
{
	struct sm5708_charger_data *charger = container_of(work, struct sm5708_charger_data, topoff_work.work);
	bool topoff = 1;
	int i;
...
static void _reduce_input_limit_current(struct sm5708_charger_data *charger, int cur)
{
	...
	vbus_limit_current = ((vbus_limit_current - cur) < MINIMUM_INPUT_CURRENT) ?	MINIMUM_INPUT_CURRENT : vbus_limit_current - cur;
	sm5708_CHG_set_INPUT_LIMIT(charger, vbus_limit_current);

	charger->charging_current_max = sm5708_get_input_current(charger);

	pr_info("vbus_limit_current=%d, charger->charging_current_max=%d\n",
		vbus_limit_current, charger->charging_current_max);
}

static void _check_slow_charging(struct sm5708_charger_data *charger, int input_current)
{
...
{
	struct sm5708_charger_data *charger = container_of(work, struct sm5708_charger_data, aicl_work.work);
...
static int _check_vbus_power_supply_status(struct sm5708_charger_data *charger,	unsigned char vbus_status, int prev_battery_health)
{
...
#ifdef CONFIG_OF
static int _parse_sm5708_charger_node_propertys(struct device *dev,	struct device_node *np, sec_charger_platform_data_t *pdata)
{
...
static int sm5708_charger_parse_dt(struct sm5708_charger_data *charger,	struct sec_charger_platform_data *pdata)
{
...
static sec_charger_platform_data_t *_get_sm5708_charger_platform_data(struct platform_device *pdev, struct sm5708_charger_data *charger)
{
#ifdef CONFIG_OF
