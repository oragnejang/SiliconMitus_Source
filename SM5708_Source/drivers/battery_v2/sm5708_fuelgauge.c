...
static bool sm5708_fg_reg_init(struct i2c_client *client, int is_surge)
{
	...
	for (i = 0; i < TABLE_MAX; i++) {
		...
		for (j = 0; j <= SM5708_FG_TABLE_LEN; j++) {
			sm5708_fg_i2c_write_word(client, (table_reg + j), fuelgauge->info.battery_table[i][j]);
			msleep(10);
			if (fuelgauge->info.battery_table[i][j] != sm5708_fg_i2c_read_word(client, (table_reg + j))) {
