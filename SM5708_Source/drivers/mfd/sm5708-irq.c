...
static int sm5708_mask_all_irqs(struct sm5708_dev *sm5708)
{
	...
	for (i = 0; i < ARRAY_SIZE(sm5708_mask_reg); i++) {
		...
		if (rc < 0) {
			pr_info("Error : can't write reg[0x%x] = 0x%x\n", sm5708_mask_reg[i], sm5708_irqs_ctrl_mask_all_val[i]);
			return rc;
		}
	}
...
int sm5708_irq_init(struct sm5708_dev *sm5708)
{
	...
	ret = request_threaded_irq(sm5708->irq, NULL, sm5708_irq_thread, IRQF_TRIGGER_LOW | IRQF_ONESHOT | IRQF_NO_SUSPEND,
		"sm5708-irq", sm5708);
	if (ret) {
