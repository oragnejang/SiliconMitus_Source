...
#ifdef CONFIG_JZ_BATTERY_ADC_NTC
#include <linux/power/jz-battery-ntc.h>
#endif
...
#if defined(CONFIG_USB_JZ_DWC2) && defined(CONFIG_DWC2_BOND_TO_CHARGER)
extern void dwc2_gadget_plug_change(int plugin);
#endif

#ifdef CONFIG_TOUCHSCREEN_ITE7259
extern void ite7259_set_usb_plug_state(int plugin);
#endif

extern struct sm5007_fuelgauge_info *sm5007_fuelgauge;
struct sm5007_charger_info *sm5007_charger = NULL;

static void sm5007_enable_autostop(struct sm5007_charger_info *info,
		int onoff)
{
#ifdef CONFIG_SM5007_FG_DEBUG_LOG
	printk("%s:[BATT] Autostop set(%d)\n", __func__, onoff);
#endif
	if (onoff)
...
{
#ifdef CONFIG_SM5007_FG_DEBUG_LOG
	printk("%s:[BATT] RECHG threshold(%d)\n", __func__, threshold);
#endif
...
{
	uint8_t temp = 0, reg_val = 0, data = 0;
...
#ifdef CONFIG_SM5007_FG_DEBUG_LOG
	printk("%s : SM5007_CHGCNTL3 (topoff current) : 0x%02x\n",
		__func__, data);
#endif
}
...
#ifdef CONFIG_SM5007_FG_DEBUG_LOG
	printk("%s : SM5007_CHGCNTL3 (fastchg current) : 0x%02x\n",
		__func__, data);
#endif
}
...
#ifdef CONFIG_SM5007_FG_DEBUG_LOG
	printk("%s : SM5007_CHGCNTL3 (Battery regulation voltage) : 0x%02x\n",
		__func__, data);
#endif
}

int get_power_supply_status(struct sm5007_charger_info *info)
{
	uint8_t status;
	int ret = 0;
	...
	if ((status & SM5007_STATE_TOPOFF) || (status & SM5007_STATE_DONE)) {
		info->chg_status = POWER_SUPPLY_STATUS_FULL;
#ifdef CONFIG_SM5007_FG_DEBUG_LOG
		printk("%s : Status, Power Supply Full \n", __func__);
#endif
	}else if ((status & SM5007_STATE_CHGON) || (status & SM5007_STATE_VBUSPOK)) {
		info->chg_status = POWER_SUPPLY_STATUS_CHARGING;
	}else if ((status & SM5007_STATE_VBATOVP) || (status & SM5007_STATE_VBUSOVP)) {
		info->chg_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
	}else if (status & SM5007_STATE_VBUSUVLO) {
		info->chg_status = POWER_SUPPLY_STATUS_DISCHARGING;
	}else {
#ifdef CONFIG_SM5007_FG_DEBUG_LOG
		printk("%s : Status, ELSE \n", __func__);
#endif
	}
...
static int get_power_supply_Android_status(struct sm5007_charger_info *info)
{
	...
	case POWER_SUPPLY_STATUS_FULL:
		// psy_fg_do_property("sm5007-fuelgauge", get, POWER_SUPPLY_PROP_CHARGE_FULL, value);
		if (NULL != sm5007_fuelgauge) {
			if (sm5007_fuelgauge->is_charging) { //If charging is ture, status is changed to FULL
				return POWER_SUPPLY_STATUS_CHARGING;
			} else {
...
int sm5007_chg_get_prop(struct power_supply *psy,
...
int sm5007_chg_set_prop(struct power_supply *psy,
				enum power_supply_property psp,
				union power_supply_propval *val)
{
	struct sm5007_charger_info *info = dev_get_drvdata(psy->dev->parent);
...
static irqreturn_t sm5703_chg_vbuspok_irq_handler(int irq, void *data)
{
	...
	printk("%s: VBUSPOK Interrupt Occur\n", __func__);
	ret = sm5007_read(info->dev->parent, SM5007_STATUS, &status);
	if (ret < 0) {
		dev_err(info->dev, "Error in reading the control register\n");
		return ret;
	...
	if (status & SM5007_STATE_VBUSPOK) {
		...
		printk("%s, enable charge\n", __func__);
#if defined(CONFIG_USB_JZ_DWC2) && defined(CONFIG_DWC2_BOND_TO_CHARGER)
//turn on dwc2
		dwc2_gadget_plug_change(1);
#endif
#ifdef CONFIG_TOUCHSCREEN_ITE7259
		ite7259_set_usb_plug_state(1);
#endif
#ifdef CONFIG_SM5007_CHARGER_HOLD_WAKE_LOCK
...
#endif
#ifdef CONFIG_JZ_BATTERY_ADC_NTC
		info->ntc.need_check = true;
		wake_up_interruptible(&info->ntc.wq);
#endif
	} else {
#ifdef CONFIG_SM5007_CHARGER_HOLD_WAKE_LOCK
...
static irqreturn_t sm5703_chg_vbusuvlo_irq_handler(int irq, void *data)
{
	...
	printk("%s: VBUSUVLO interrupt occur\n", __func__);
	ret = sm5007_read(info->dev->parent, SM5007_STATUS, &status);
	if (ret < 0) {
		dev_err(info->dev, "Error in reading the control register\n");
		return ret;
	...
	if (status & SM5007_STATE_VBUSUVLO) {
		...
		power_supply_changed(&info->charger);
		printk("%s : disable charge\n", __func__);
#if defined(CONFIG_USB_JZ_DWC2) && defined(CONFIG_DWC2_BOND_TO_CHARGER)
//turn off dwc2
		dwc2_gadget_plug_change(0);
#endif
#ifdef CONFIG_TOUCHSCREEN_ITE7259
		ite7259_set_usb_plug_state(0);
#endif
#ifdef CONFIG_SM5007_CHARGER_HOLD_WAKE_LOCK
		if(smsuspend_lock_locked == 1){
			wake_unlock(&smsuspend_lock);
...
#ifdef CONFIG_SM5007_FG_DEBUG_LOG
		printk("%s : Register IRQ%d(%s) successfully\n",
...
#ifdef CONFIG_JZ_BATTERY_ADC_NTC
static void ntc_print_power_status(struct sm5007_charger_info *info)
{
	int status = 0;
	...
	switch (status) {
	case POWER_SUPPLY_STATUS_CHARGING:
		printk(NTC_LOG_LEVEL "charger status : charging\n");
		break;
	case POWER_SUPPLY_STATUS_DISCHARGING:
		printk(NTC_LOG_LEVEL "charger status : discharging\n");
		break;
	case POWER_SUPPLY_STATUS_NOT_CHARGING:
		printk(NTC_LOG_LEVEL "charger status : not_charging\n");
		break;
	case POWER_SUPPLY_STATUS_FULL:
		printk(NTC_LOG_LEVEL "charger status : full\n");
		break;
	default:
		printk(NTC_LOG_LEVEL "charger status : unknown\n");
		break;
	}
}

static int ntc_get_avg_volt(void)
{
	int i = 0;
	int volt = 0;

	for (i = 0; i < NTC_EVERY_CHECK_COUNT; i++) {
		volt += jz_battery_get_ntc_volt();
		msleep(NTC_EVERY_CHECK_INTERVAL);
	}
	volt = volt / NTC_EVERY_CHECK_COUNT;

	return volt;
}

static int ntc_query_index(unsigned long table[], unsigned long value)
{
	int low = 0;
	int high = NTC_R_TABLE_NUM - 1;
	int mid = 0;
	unsigned long mid_val = 0;

	if (value > table[0])
		return 0;

	if (value < table[NTC_R_TABLE_NUM-1])
		return NTC_R_TABLE_NUM - 1;
...
static int ntc_get_temp_by_volt(int volt)
{
	unsigned long r = 0;
	int index = 0;
	int temp = 0;

	/* we need convert ntc voltage into r */
	r = NTC_R_ENLARGE_MULTIPLE * 100 * volt / (ADC_REF_VOL - volt);
	printk(NTC_LOG_LEVEL "%s : NTC volt = %d mv R = %ld ohm\n", __func__, volt, r/10);

	index = ntc_query_index(ntc_r_table, r);
	temp = index + NTC_R_MIN_TEMP;
	printk(NTC_LOG_LEVEL "%s : temp = %d\n", __func__, temp);

	return temp;
}

static void ntc_set_charging_current(void *data, int temp)
{
	int charging_current = 0;
	struct sm5007_charger_info *info = data;

	printk("wqkong >>>> return ntc_set_charging_current directly, do nothing\n");
	return;

	if (temp < 0) {
		charging_current = 200;
		printk(NTC_LOG_LEVEL "%s : temp < 0c, set current(%dma)\n", __func__, charging_current);
	} else if (temp >= 0 && temp < 15) {
		charging_current = BATTERY_CAPACITY * 3 / 10;
		printk(NTC_LOG_LEVEL "%s : 0c <= temp < 15c, set current(%dma)\n", __func__, charging_current);
	} else if (temp >= 15 && temp <= 45) {
		charging_current = BATTERY_CAPACITY;
		printk(NTC_LOG_LEVEL "%s : 15c <= temp <= 45c, set current(%dma)\n", __func__, charging_current);
	} else if (temp > 45) {
		charging_current = 200;
		printk(NTC_LOG_LEVEL "%s : temp > 45c, set current(%dma)\n", __func__, charging_current);
	}

	sm5007_set_fast_charging_current(info, charging_current);
}

static int ntc_check_thread(void *data)
{
	struct sm5007_charger_info *info = data;
	static int check_count = 0;
	int volt = 0;
	int temp = 0;
	int ntc_check_interval = NTC_CHECK_INTERVAL;
	int i = 0;

	while(!kthread_should_stop()) {
		/* wait for plugin usb, need set charging current at once */
		wait_event_interruptible(info->ntc.wq, info->ntc.need_check);

		/* can't advance adc driver, so just a workaround solution */
		if (check_count == 0) {
			printk(NTC_LOG_LEVEL "%s : wait adc driver ready\n", __func__);
			check_count++;
			msleep(2000);
		}

		ntc_print_power_status(info);

		/* just need read ntc and set current when charging */
		if (POWER_SUPPLY_STATUS_CHARGING == info->chg_status) {
			volt = ntc_get_avg_volt();
			temp = ntc_get_temp_by_volt(volt);
			ntc_set_charging_current(data, temp);
		}

		printk(NTC_LOG_LEVEL "%s : wait %ds for next ntc check\n", __func__, ntc_check_interval);
		for (i = 0; i < NTC_CHECK_INTERVAL; i++) {
			if (wait_event_interruptible_timeout(info->ntc.wq,
					(!info->ntc.need_check), NTC_TIMEOUT)) {
				printk(NTC_LOG_LEVEL "%s : usb plugout, stop sleep\n", __func__);
				break;
			}
		}
	}
...
static int sm5007_charger_probe(struct platform_device *pdev)
{
	...
	info->charger.type = POWER_SUPPLY_TYPE_BATTERY;
	...
	info->usbpwr.name = "sm5007-usbpwr";
	info->usbpwr.type = POWER_SUPPLY_TYPE_USB;
	info->usbpwr.properties = sm5007_power_props;
	info->usbpwr.num_properties = ARRAY_SIZE(sm5007_power_props);
	info->usbpwr.get_property = sm5007_chg_get_prop;
	info->usbpwr.set_property = sm5007_chg_set_prop;

	ret = power_supply_register(&pdev->dev, &info->usbpwr);
	if (ret)
		info->usbpwr.dev->parent = &pdev->dev;
...
#ifdef CONFIG_JZ_BATTERY_ADC_NTC
	info->ntc.thread = kthread_run(ntc_check_thread, info, "ntc_check");
	if (IS_ERR(info->ntc.thread)) {
		dev_err(info->dev, "Failed to run ntc check thread\n");
		goto err_reg_irq;
	}
	init_waitqueue_head(&info->ntc.wq);
#endif
