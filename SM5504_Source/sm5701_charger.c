...
static int SM5701_charger_probe(struct platform_device *pdev)
{
	...
	/* CHG_EN is low bydefault, make it high in sleep and no charger connected case */
	if (gpio_is_valid(pdata->chgen))
	{
		ret = gpio_direction_output(pdata->chgen, 1);
		if(ret)
			pr_err("%s : gpio direction output error, ret = %d", __func__, ret);
	}
	else
	{
		pr_err("%s : Charger GPIO(%d) not correct\n", __func__,pdata->chgen);
	}
