...
static struct sm5010_buck_info *static_info = NULL;

/* In case of Freq = 0 */
enum sm5010_ramp0_rate {
	RAMP_RATE_2P343MV,
	RAMP_RATE_4P69MV,
	RAMP_RATE_9P38MV,
	RAMP_RATE_18P76MV,
...
struct sm5010_buck_info {	
	struct device			*dev;
	struct sm5010_pmic_dev *iodev;
	struct regulator_dev *rdev[SM5010_BUCKMAX];
	
	int dvs_pin[SM5010_DVSPINS_MAX]; /* DVS GPIO */
...
static int _sm5010_buck_is_enabled(struct regulator_dev *rdev)
{
	struct sm5010_buck_info *sm5010 = rdev_get_drvdata(rdev);
	...
	switch (reg_id) {
		case SM5010_PABUCK:				
			enabled_val = (reg_sta != 0);
			break;			
		case SM5010_BUCK2 ... SM5010_BUCK6:						
 ...
 * mode 2 : SM5010_PWM_OPMODE_AUTOMODE
		1 : SM5010_PWM_OPMODE_FORCEPWMMODE
 */
				     ...
				     unsigned int mode)
{
	...
	switch (reg_id) {		
	case SM5010_BUCK2:
		mode_reg = SM5010_REG_BUCK2CNTL3;		
		break;		
	case SM5010_BUCK3:
		mode_reg = SM5010_REG_BUCK3CNTL3;		
		break;		
	case SM5010_BUCK4:
		mode_reg = SM5010_REG_BUCK4CNTL3;		
		break;		
	case SM5010_BUCK5:
		mode_reg = SM5010_REG_BUCK5CNTL3;		
		break;		
	case SM5010_BUCK6:
		mode_reg = SM5010_REG_BUCK6CNTL3;		
		break;
	case SM5010_PABUCK:
		mode_reg = SM5010_REG_PABUCKCNTL2;		
		break;		
	default:
	...
	if (mode == REGULATOR_MODE_FAST)
		val = SM5010_PWM_OPMODE_FORCEPWMMODE;
	else if (mode == REGULATOR_MODE_NORMAL)
		val = SM5010_PWM_OPMODE_AUTOMODE;
	else
		return -EINVAL;

	ret = sm5010_reg_update(sm5010->iodev, mode_reg,
				  val << SM5010_PWM_OPMODE_SHIFT, SM5010_PWM_OPMODE_MASK);
	if (ret)
		return ret;
...
static unsigned int sm5010_get_mode(struct regulator_dev *rdev)
{
	...
	ret = sm5010_reg_read(sm5010->iodev, mode_reg, &val);
	if (ret)
		return ret;
...
static int sm5010_enable(struct regulator_dev *rdev)
{
	...
	switch (reg_id)
	{
		case SM5010_PABUCK:				
			break;			
		case SM5010_BUCK2 ... SM5010_BUCK6:
			/* ENnPWRSTM Bit is 1 and buck is always controlled by nPWRSTM , register is set to SM5010_OPMODE_nPWRSTM */
...
{
	return _sm5010_buck_is_enabled(rdev);
}
...
#define	RAMP_SHIFT	3
static int sm5010_set_ramp_delay(struct regulator_dev *rdev, int ramp_delay)
{
	struct sm5010_buck_info *sm5010 = rdev_get_drvdata(rdev);
	int reg_id = rdev_get_id(rdev);
	...
	case SM5010_BUCK2:
		...
		ramp_reg = SM5010_REG_BUCK2CNTL3;		
		break;		
	case SM5010_BUCK3:
		ramp_value = get_ramp_delay(ramp_delay, 1);
		ramp_reg = SM5010_REG_BUCK3CNTL3;		
		break;		
	case SM5010_BUCK4:
		ramp_value = get_ramp_delay(ramp_delay, 1);
		ramp_reg = SM5010_REG_BUCK4CNTL3;		
		break;		
	case SM5010_BUCK5:
		ramp_value = get_ramp_delay(ramp_delay, 1);
		ramp_reg = SM5010_REG_BUCK5CNTL3;		
		break;		
	case SM5010_BUCK6:
		ramp_value = get_ramp_delay(ramp_delay, 1);
		ramp_reg = SM5010_REG_BUCK6CNTL3;		
		break;
	default:
		return -EINVAL;
	}

	return sm5010_reg_update(sm5010->iodev, ramp_reg,
...
static int sm5010_set_voltage_time_sel(struct regulator_dev *rdev,
...
static struct regulator_ops sm5010_buck_ops = {
	...
	.set_voltage_time_sel	= sm5010_set_voltage_time_sel,
	.set_ramp_delay		= sm5010_set_ramp_delay,
	.set_mode		= sm5010_set_mode,
	.get_mode		= sm5010_get_mode,
};

#define _BUCK(macro)	SM5010_BUCK##macro
#define _buck_ops(num)	sm5010_buck_ops##num
#define _REG_BUCKxCNTLx(num1, num2)	SM5010_REG_BUCK##num1##CNTL##num2

#define _PABUCK(macro)	SM5010_PABUCK##macro
#define _buck_ops(num)	sm5010_buck_ops##num
#define _REG_PABUCKCNTLx(num)	SM5010_REG_PABUCKCNTL##num

#define _REG(ctrl)	SM5010_REG##ctrl


#define SM5010_BUCK_DESC(_name, _id, _ops, _mu, _us, _nv, _vr, _vm, _er, _em)	{	\
	.name		= _name,				\
	.id			= _id,					\
	.ops		= _ops,					\
...
static struct regulator_desc sm5010_bucks_desc[SM5010_BUCKMAX] = {
	/* name, id, ops, min_uv, uV_step, n_voltages, vsel_reg, vsel_mask, enable_reg, enable_mask */
	SM5010_BUCK_DESC("BUCK2", _BUCK(2), &_buck_ops(), _BUCK(_MIN_0P6V), _BUCK(_STEP_12P5MV), _BUCK(_NUM_112),
					_BUCK(_VSEL_MASK_FF), _REG_BUCKxCNTLx(2,2), _REG_BUCKxCNTLx(2,1), _BUCK(_ENABLE_MASK_03)),
	SM5010_BUCK_DESC("BUCK3", _BUCK(3), &_buck_ops(), _BUCK(_MIN_0P6V), _BUCK(_STEP_12P5MV), _BUCK(_NUM_112), 
					_BUCK(_VSEL_MASK_FF), _REG_BUCKxCNTLx(3,2), _REG_BUCKxCNTLx(3,1), _BUCK(_ENABLE_MASK_03)),
	SM5010_BUCK_DESC("BUCK4", _BUCK(4), &_buck_ops(), _BUCK(_MIN_0P6V), _BUCK(_STEP_12P5MV), _BUCK(_NUM_112),
					_BUCK(_VSEL_MASK_FF), _REG_BUCKxCNTLx(4,2), _REG_BUCKxCNTLx(4,1), _BUCK(_ENABLE_MASK_03)),
	SM5010_BUCK_DESC("BUCK5", _BUCK(5), &_buck_ops(), _BUCK(_MIN_0P6V), _BUCK(_STEP_12P5MV), _BUCK(_NUM_112), 
					_BUCK(_VSEL_MASK_FF), _REG_BUCKxCNTLx(5,2), _REG_BUCKxCNTLx(5,1), _BUCK(_ENABLE_MASK_03)),
	SM5010_BUCK_DESC("BUCK6", _BUCK(6), &_buck_ops(), _BUCK(_MIN_0P6V), _BUCK(_STEP_12P5MV), _BUCK(_NUM_112),
					_BUCK(_VSEL_MASK_FF), _REG_BUCKxCNTLx(6,2), _REG_BUCKxCNTLx(6,1), _BUCK(_ENABLE_MASK_03)),
					
	SM5010_BUCK_DESC("PABUCK", _PABUCK(), &_buck_ops(), _BUCK(_MIN_0P5XV), _BUCK(_STEP_12P5MV), _BUCK(_NUM_255),
					_BUCK(_VSEL_MASK_FF), _REG_PABUCKCNTLx(1), _REG_PABUCKCNTLx(2), _BUCK(_ENABLE_MASK_10)),					 										
};

#ifdef CONFIG_OF
static int sm5010_buck_dt_parse_pdata(struct sm5010_pmic_dev *iodev,
					struct sm5010_platform_data *pdata)
{
	...
	buck_np = of_find_node_by_name(pmic_np, "sm5010-buck");
	...
	for_each_child_of_node(bucks_np, reg_np) {
		for (i = 0; i < ARRAY_SIZE(sm5010_bucks_desc); i++)
			if (!of_node_cmp(reg_np->name, sm5010_bucks_desc[i].name))
				break;

		if (i == ARRAY_SIZE(sm5010_bucks_desc)) {
		...
		rdata->initdata = of_get_regulator_init_data(
						iodev->dev, reg_np, &sm5010_bucks_desc[i]);
...
static int sm5010_buck_preinit(struct sm5010_buck_info *info)
{
	struct sm5010_pmic_dev *iodev = info->iodev;
	...
	if (iodev->rev_num == 0)
	{
		ret = sm5010_reg_update(iodev, SM5010_REG_BUCK2CNTL3, (pdata->buck2mode & 0x1), SM5010_PWM_OPMODE_MASK);
		if (ret < 0) {
			printk("%s : BUCK2MODE Pre-Init failed\n",__func__);
			return ret;
		}
		ret = sm5010_reg_update(iodev, SM5010_REG_BUCK3CNTL3, (pdata->buck3mode & 0x1), SM5010_PWM_OPMODE_MASK);
		if (ret < 0) {
			printk("%s : BUCK3MODE Pre-Init failed\n",__func__);
			return ret;
		}		
		ret = sm5010_reg_update(iodev, SM5010_REG_BUCK4CNTL3, (pdata->buck4mode & 0x1), SM5010_PWM_OPMODE_MASK);
		if (ret < 0) {
			printk("%s : BUCK4MODE Pre-Init failed\n",__func__);
			return ret;
		}		
		ret = sm5010_reg_update(iodev, SM5010_REG_BUCK5CNTL3, (pdata->buck5mode & 0x1), SM5010_PWM_OPMODE_MASK);
		if (ret < 0) {
			printk("%s : BUCK5MODE Pre-Init failed\n",__func__);
			return ret;
		}		
		ret = sm5010_reg_update(iodev, SM5010_REG_BUCK6CNTL3, (pdata->buck6mode & 0x1), SM5010_PWM_OPMODE_MASK);
		if (ret < 0) {
...
static int sm5010_buck_probe(struct platform_device *pdev)
{
	struct sm5010_pmic_dev *iodev = dev_get_drvdata(pdev->dev.parent);
	struct sm5010_platform_data *pdata = iodev->pdata;
	struct regulator_config config = { };
	struct sm5010_buck_info *info;
	int i;
	int ret = 0;

	pr_info("%s : Start\n", __func__);

	info = devm_kzalloc(&pdev->dev, sizeof(struct sm5010_buck_info),
				GFP_KERNEL);
	if (!info)
		return -ENOMEM;
	...
	if (iodev->dev->of_node) {
		ret = sm5010_buck_dt_parse_pdata(iodev, pdata);
		if (ret) {
			return ret;
		}
	}
	...
	for (i = 0; i < pdata->num_bucks; i++) {
		...
		info->rdev[i] = regulator_register(&sm5010_bucks_desc[id], &config);
...
err:
	for (i = 0; i < SM5010_BUCKMAX; i++)
		regulator_unregister(info->rdev[i]);

	return ret;
}

static int sm5010_buck_remove(struct platform_device *pdev)
{
	struct sm5010_buck_info *info = platform_get_drvdata(pdev);
	int i;
...
static const struct of_device_id sm5010_buck_of_match[] = {
	{ .compatible = "sm,sm5010-buck", },
	{ },
};
MODULE_DEVICE_TABLE(platform, sm5010_buck_of_match);

static const struct platform_device_id sm5010_buck_id[] = {
	{ "sm5010-buck", 0 },
};

static struct platform_driver sm5010_buck_driver = {
	.driver = {
		.name = "sm5010-buck",
		.owner = THIS_MODULE,
		.of_match_table = sm5010_buck_of_match,		
	},
	.probe = sm5010_buck_probe,
	.remove = sm5010_buck_remove,
	.id_table = sm5010_buck_id,
};

static int __init sm5010_buck_init(void)
{
	pr_info("%s\n", __func__);

	return platform_driver_register(&sm5010_buck_driver);
}
module_init(sm5010_buck_init);

static void __exit sm5010_buck_exit(void)
{
	platform_driver_unregister(&sm5010_buck_driver);
}
module_exit(sm5010_buck_exit);

MODULE_DESCRIPTION("SM5010 Regulator Buck Driver");
MODULE_LICENSE("GPL");
